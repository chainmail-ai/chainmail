...
\usepackage{cite}
\usepackage{graphicx}
%% SVG figures — Overleaf: Menu → Compiler → set "Shell escape" = enabled
\usepackage{svg}
...
%% ─── Hyphenation fixes ───────────────────────────────────────────────────────
\hyphenation{uni-fied Chain-mail SBOM VEX CVSS CSAF SLSA PURL npm PyPI}
...
\date{May 2026 \\[0.4ex]
\normalsize\textit{Zenodo preprint} \quad
\texttt{cs.SE} (primary) $\cdot$ \texttt{cs.CR} (cross-list) \\[0.3ex]
\normalsize\texttt{doi:10.5281/zenodo.19934776}}
...
\url{https://chainmail.saisravancherukuri.com}.
...
Orval codegen. Figure~\ref{fig:arch} illustrates the full system.

\begin{figure*}[t]
  \centering
  \includesvg[inkscapelatex=false,width=\textwidth]{figure1}
  \caption{Chainmail system architecture. The analysis pipeline ingests repository
  metadata and produces eight security artifacts, each exposed through a typed REST
  endpoint. The OpenAPI specification serves as the single source of truth for both
  server validation and client code generation.}
  \label{fig:arch}
\end{figure*}
...
\toprule
\textbf{Grade} & \textbf{SCRS} & \textbf{Risk} & \textbf{Recommended Action} \\
\midrule
A & 0--19   & Excellent & Routine monitoring \\
B & 20--39  & Good      & Schedule updates within 90 days \\
C & 40--59  & Fair      & Resolve HIGH/CRITICAL CVEs within 30 days \\
D & 60--79  & Poor      & Immediate remediation plan required \\
...
\begin{figure}[H]
  \centering
  \includesvg[inkscapelatex=false,width=\columnwidth]{figure2}
  \caption{SCRS radar visualization for facebook/react. The five-axis chart
  makes per-dimension risk immediately visible. Inner dashed circle = grade B
  threshold; outer ring = grade F. Vulnerability and Freshness dominate by
  design, reflecting their higher incident correlation weights.}
  \label{fig:radar}
\end{figure}
...
\{{\normalfont\textsc{Compatible}},\allowbreak{\normalfont\textsc{Incompatible}},\allowbreak{\normalfont\textsc{Warning}}\}$
encodes legal compatibility between any two licenses. $C$ is not symmetric in
general: MIT $\to$ GPL-3.0 is {\normalfont\textsc{Compatible}}, but GPL-3.0 $\to$ MIT is
{\normalfont\textsc{Incompatible}}.
\end{definition}
...
\begin{figure*}[t]
  \centering
  \includesvg[inkscapelatex=false,width=\textwidth]{figure3}
  \caption{License Conflict Propagation DAG for a representative project. Nodes
  show package name and SPDX license; edge color encodes compatibility result
  (green = COMPATIBLE, red = INCOMPATIBLE). The GPL-3.0 dependency at depth 2
  propagates a conflict invisible to flat-list license scanners.}
  \label{fig:licensedag}
\end{figure*}

\FloatBarrier
%% ─── SECTION 6 ───────────────────────────────────────────────────────────────
% Figure 4 placed here so LaTeX floats it to the top of the Section 6 page,
% avoiding the large gap that occurs when it is defined after the algorithm.
\begin{figure*}[t]
  \centering
  \includesvg[inkscapelatex=false,width=\textwidth]{figure4}
  \caption{VEX generation pipeline. The process flows from SBOM ingestion
  through PURL normalization, CVE matching, version-range intersection,
  exploitability classification, and CSAF~2.0 output. Human review is required
  only for UNDER\_INVESTIGATION cases, which constitute fewer than 15\% of
  matched (CVE, component) pairs in practice.}
  \label{fig:vexpipeline}
\end{figure*}
...
\begin{algorithmic}[1]
\Require SBOM $S = \{c_1,\ldots,c_n\}$, CVE DB $\mathcal{V}$, PURL index $\Pi$
\Ensure VEX statement set $X$
\State $X \gets \emptyset$
\ForAll{$c_i \in S$}
  \State $\mathrm{purl}_i \gets \Pi(c_i)$
  \State $M \gets \{v \in \mathcal{V} : \mathrm{eco}(v) = \mathrm{eco}(c_i)\}$
  \ForAll{$v \in M$}
...
\begin{algorithm}[H]
\caption{$\textsc{ClassifyStatus}(v, c_i)$ --- subroutine of Alg.~2}
\begin{algorithmic}[1]
\If{$\mathrm{IsPatched}(c_i, v)$}
  \Return \textsc{Fixed}
\EndIf
\If{$\lnot\,\mathrm{HasCodePath}(c_i, v)$}
  \Return \textsc{NotAffected}
\EndIf
\If{$\mathrm{CVSS\_AV}(v) = \textsc{Physical}$}
  \Return \textsc{NotAffected}
\EndIf
...
repository URL to \url{https://chainmail.saisravancherukuri.com} and get a full supply chain security
